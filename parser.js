 ...
 class MerkabahParser {
 	...
 	_consumeSemicolon() {
 		if (this._currTokenIs(TOKEN.SEMICOLON)) {
 			this._advance()
 		} else if (!this.currToken.hasLineTerminatorBefore && !this._currTokenIs(TOKEN.RBRACE) && !this._currTokenIs(TOKEN.EOF)) {}
 	}
 	_getPrecedence(t) {
 	...
 	_registerPrefixFns() {
 		return {
 			...
 			[TOKEN.TEMPLATE_TAIL]: this._parseTemplateLiteral,
 		}
 	}
 	_registerInfixFns() {
 		...
 		return {
 			...
 			[TOKEN.ASSIGN]: a,
 			[TOKEN.PLUS_ASSIGN]: a,
 			[TOKEN.MINUS_ASSIGN]: a,
 			[TOKEN.ASTERISK_ASSIGN]: a,
 			[TOKEN.SLASH_ASSIGN]: a,
 			[TOKEN.EXPONENT_ASSIGN]: a,
 			[TOKEN.NULLISH_ASSIGN]: a,
 			[TOKEN.LPAREN]: l => this._parseCallExpression(l),
 			[TOKEN.DOT]: l => this._parseMemberExpression(l),
 			[TOKEN.OPTIONAL_CHAINING]: l => this._parseMemberExpression(l, false, true),
 	...
 	_parseMemberExpression(o, c = false, opt = false) {
 		const s = {
 			start: o.start,
 			loc: {
 				start: o.loc.start
 	...
 	_parseCallExpression(c) {
 		...
 		const a = this._parseArgumentList();
 		return this._finishNode({
 			type: 'CallExpression',
 			callee: c,
 			arguments: a,
 	...
 	_parseArgumentList() {
 		const a = [];
 		if (!this._peekTokenIs(TOKEN.RPAREN)) {
 			this._advance();
 			while (!this._currTokenIs(TOKEN.RPAREN)) {
 				a.push(this._currTokenIs(TOKEN.DOTDOTDOT) ? this._parseSpreadElement() : this._parseExpression(PRECEDENCE.ASSIGNMENT));
 				if (!this._currTokenIs(TOKEN.RPAREN)) this._expect(TOKEN.COMMA)
 			}
 		}
 	...
 	_parseTemplateLiteral() {
 		...
 		const q = [];
 		const e = [];
 		const head = this.currToken;
 		const isTailOnly = head.type === TOKEN.TEMPLATE_TAIL;
 		...
 		while (!this._currTokenIs(TOKEN.EOF)) {
 			...
 			if (this._currTokenIs(TOKEN.TEMPLATE_MIDDLE)) {
 				const mid = this.currToken;
 				q.push(this._finishNode({
 					type: 'TemplateElement',
 					value: {
 						raw: mid.literal,
 				...
 				this._advance()
 			} else if (this._currTokenIs(TOKEN.TEMPLATE_TAIL)) {
 				const tail = this.currToken;
 				q.push(this._finishNode({
 					type: 'TemplateElement',
 					value: {
 						raw: tail.literal,
 				...
 				this._error('Unterminated template literal expression');
 				break
 			}
 		}
 		return this._finishNode({
 	...
 	_parseFunction(c) {
 		const s = this._startNode();
 		const isAsync = this.currToken.type === TOKEN.ASYNC;
 		if (isAsync) this._advance();
 		this._expect(TOKEN.FUNCTION);
 		let id = null;
 		if (this._currTokenIs(TOKEN.IDENT)) id = this._parseIdentifier();
 		else if (c === 'declaration' && !this._peekTokenIs(TOKEN.LPAREN)) this._error("Function declarations require a name");
 		this._expect(TOKEN.LPAREN);
 		const p = this._parseFunctionParams();
 		this._expect(TOKEN.RPAREN);
 	...
 	_parseGroupedOrArrowExpression() {
 		...
 		for (const i of p) {
 			if (i.type !== 'Identifier' && i.type !== 'ObjectPattern' && i.type !== 'ArrayPattern') this._error("Invalid arrow function parameter")
 		}
 		const b = this._currTokenIs(TOKEN.LBRACE) ? this._parseBlockStatement() : this._parseExpression(PRECEDENCE.ASSIGNMENT);
 		return this._finishNode({
 			type: 'ArrowFunctionExpression',
 	...
 	_parseBlockStatement() {
 		...
 		this._expect(TOKEN.LBRACE);
 		const b = [];
 		while (!this._currTokenIs(TOKEN.RBRACE) && !this._currTokenIs(TOKEN.EOF)) b.push(this._parseDeclaration());
 		this._expect(TOKEN.RBRACE);
 		return this._finishNode({
 			type: 'BlockStatement',
 			body: b
 	...
 	_parseIfStatement() {
 		...
 		this._expect(TOKEN.RPAREN);
 		const c = this._parseStatement();
 		let a = null;
 		if (this._currTokenIs(TOKEN.ELSE)) {
 			this._advance();
 			a = this._parseStatement()
 		}
 		return this._finishNode({
 			type: 'IfStatement',
 			test: t,
 	...
 	_parseForStatement() {
 		...
 		let i = null,
 			t, u, k = false;
 		if (!this._currTokenIs(TOKEN.SEMICOLON)) {
 			i = this._currTokenIs(TOKEN.LET) || this._currTokenIs(TOKEN.CONST) || this._currTokenIs(TOKEN.VAR) ? this._parseVariableDeclaration() : this._parseExpression(PRECEDENCE.LOWEST)
 		}
 		...
 		t = !this._currTokenIs(TOKEN.SEMICOLON) ? this._parseExpression(PRECEDENCE.LOWEST) : null;
 		this._expect(TOKEN.SEMICOLON);
 		u = !this._currTokenIs(TOKEN.RPAREN) ? this._parseExpression(PRECEDENCE.LOWEST) : null;
 		this._expect(TOKEN.RPAREN);
 		const b = this._parseStatement();
 		return this._finishNode({
 			type: 'ForStatement',
 	...
 	_parseReturnStatement() {
 		...
 		this._advance();
 		let a = null;
 		if (!this._currTokenIs(TOKEN.SEMICOLON) && !this.currToken.hasLineTerminatorBefore) a = this._parseExpression(PRECEDENCE.LOWEST);
 		this._consumeSemicolon();
 		return this._finishNode({
 	...
 	_parseExportDeclaration() {
 		...
 		if (this._currTokenIs(TOKEN.ASTERISK)) {
 			this._error("Export all ('export *') is not yet supported in this context.");
 			return null
 		}
