...
class MerkabahParser {
	...
	_parseTemplateLiteral() {
		...
		this._advance();
		while (!this._currTokenIs(TOKEN.TEMPLATE_TAIL) && !this._currTokenIs(TOKEN.EOF)) {
			e.push(this._parseExpression(PRECEDENCE.LOWEST));
			this.l.templateStack.push(1);
			const nextToken = this.l.nextToken();
			quasi = this._startNode();
			q.push(this._finishNode({
				type: 'TemplateElement',
				value: {
					raw: nextToken.literal,
					cooked: nextToken.literal
				},
				tail: nextToken.type === TOKEN.TEMPLATE_TAIL
			}, quasi));
			this.currToken = nextToken;
			this._advance();
			if (nextToken.type === TOKEN.TEMPLATE_TAIL) break
		}
		return this._finishNode({
	...
	_parseGroupedOrArrowExpression() {
		...
		if (this._currTokenIs(TOKEN.RPAREN)) {
			...
			if (!this._currTokenIs(TOKEN.ARROW)) {
				this._error("Expected => for arrow function with no parameters");
				return this._finishNode({
	...
	_parseVariableDeclarator() {
		...
		} else if (id.type !== 'Identifier' && (this.parentKind === 'for' || this.parentKind === 'for-of')) {
			this._error('Destructuring declarations in for loops must have an initializer.');
		}
		return this._finishNode({
	...
	_parseForStatement() {
		...
		else if (!this._currTokenIs(TOKEN.SEMICOLON)) i = this._parseExpression(PRECEDENCE.LOWEST);
		...
		t = this._currTokenIs(TOKEN.SEMICOLON) ? null : this._parseExpression(PRECEDENCE.LOWEST);
		this._expect(TOKEN.SEMICOLON);
		u = this._currTokenIs(TOKEN.RPAREN) ? null : this._parseExpression(PRECEDENCE.LOWEST);
		this._expect(TOKEN.RPAREN);
		const b = this._parseStatement();
		return this._finishNode({
			type: 'ForStatement',
	...
	_parseReturnStatement() {
		...
		if (!this._currTokenIs(TOKEN.SEMICOLON) && !this.currToken.hasLineTerminatorBefore) {
			a = this._parseExpression(PRECEDENCE.LOWEST);
		}
		this._consumeSemicolon();
		return this._finishNode({
